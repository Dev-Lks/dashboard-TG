-- =====================================================
-- Appointment attendance tracking (realizado / faltou)
-- =====================================================

alter table public.appointments
  add column if not exists attendance_status text not null default 'pending'
    check (attendance_status in ('pending', 'completed', 'no_show')),
  add column if not exists completed_at timestamptz;

-- Backfill: confirmed appointments stay pending; cancelled keep default (ignored in UI)
update public.appointments
set attendance_status = 'pending'
where status = 'confirmed' and attendance_status = 'pending';

-- One active booking per volunteer per mission (pending or completed only)
drop index if exists public.appointments_one_confirmed_per_volunteer_mission;
...
create unique index if not exists appointments_one_confirmed_per_volunteer_mission
  ...
  where (status = 'confirmed' and attendance_status in ('pending', 'completed'));

-- =====================================================
-- Updated create_appointment: ignore no_show for re-booking
-- =====================================================
...
comment on function public.create_appointment is 'Atomic appointment creation. Enforces: 1 active booking per volunteer per mission (pending/completed), capacity per date, schedule_mode slot rules.';
