...
import { UNIT_LABEL } from '@/lib/branding';
...
export function EventHero({ mission, dates }: EventHeroProps) {
    ...
    return (
      <section className="mission-panel rounded-lg px-5 py-8 sm:px-8 sm:py-10">
        <div className="max-w-2xl">
          <p className="panel-eyebrow text-xs font-extrabold uppercase tracking-[0.18em]">{mission.name}</p>
          ...
          <p className="panel-muted mt-4 text-base leading-7">
            Nenhuma data aberta no momento para esta missão.
          </p>
          <Link href="/agendar" className="panel-link mt-6 inline-block text-sm font-bold hover:underline">
            Ver outras missões
          </Link>
        </div>
      </section>
    ...
    <div className="space-y-4">
      <section className="mission-panel rounded-lg px-5 py-6 sm:px-8 sm:py-8">
        <div className="max-w-2xl">
          ...
          <h1 className="mt-2 text-3xl font-extrabold tracking-tight sm:text-4xl">{UNIT_LABEL}</h1>
          {mission.description && (
            <p className="panel-muted mt-2 text-sm">{mission.description}</p>
          )}
          ...
          {mission.location && (
            <div className="panel-muted mt-4 flex items-start gap-2 text-sm">
              ...
              <div>
                ...
                {mission.maps_url && (
                  <a href={mission.maps_url} target="_blank" rel="noopener noreferrer" className="panel-link mt-1 inline-block font-bold hover:underline">
                    Ver no mapa
                  </a>
                )}
              </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            {hasOpenSlots ? (
              <Link href={`/agendar/${mission.slug}`} className="btn btn-primary btn-lg sm:w-auto">
            ...
            {mission.phone && (
              <a href={`tel:${mission.phone.replace(/\D/g, '')}`} className="btn btn-ghost btn-lg sm:w-auto">
                <Phone className="h-4 w-4" />
                {mission.phone}
              </a>
            )}
          </div>
          <Link href="/agendar" className="panel-link mt-3 inline-block text-sm font-bold hover:underline">
            Ver outras missões
          </Link>
        </div>
      </section>
