...
export function PublicShell({ children, compact = false }: PublicShellProps) {
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      <header className="safe-top sticky top-0 z-40 border-b border-[var(--border)] bg-[var(--surface)]/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between gap-3 px-4 sm:h-16">
          <Link href="/" className="flex min-w-0 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-[var(--sand)] bg-[var(--surface-dark)] text-[var(--sand)] sm:h-10 sm:w-10">
          ...
          <nav className="flex shrink-0 items-center gap-2" aria-label="Navegação principal">
            ...
            <Link href="/admin" className="rounded border border-[var(--border)] px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--olive-dark)] hover:bg-[var(--surface-muted)] sm:px-3 sm:py-2 sm:text-xs">
              Admin
            </Link>
          </nav>
        </div>
      ...
      <main className={`mx-auto w-full max-w-5xl px-4 ${compact ? 'py-5 sm:py-8' : 'py-8 sm:py-10'}`}>
        {children}
      </main>

      <footer className="safe-bottom border-t border-[var(--border)] px-4 py-5 text-center text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">
        Operação digital: Monitor 69 Antonio
      </footer>
    </div>
  );
}
